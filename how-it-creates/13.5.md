## update Application.php — i-wire ang Session, Csrf, at Cache

```bash
Ngayong tapos na ang Session layer (13.3) at Security utils (13.4), pati na
ang Cache layer (13.1), kailangan na sila i-bind sa Container — parehong
pattern gaya ng ginawa na sa ConnectionInterface sa 13.md. Idagdag ito
kaagad pagkatapos ng 4.2 (DB binding) sa loob ng Application::run():
```
...
```bash
Pansinin: sa hakbang 6, gumagamit na tayo ng $pipeline->pipe(self::$middlewareGroups['global'] ?? [])
bago pa man i-dispatch ang router — dito rin dumadaan ang mga GLOBAL
middleware (SecurityHeadersMiddleware, CorsMiddleware) na gagawin natin sa
susunod na file (13.6.md).
```
